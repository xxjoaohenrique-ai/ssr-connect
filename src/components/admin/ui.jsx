  ...
  return (
    ...
    >
      <motion.div
        ...
        className={`max-h-[90dvh] w-full max-w-2xl overflow-y-auto rounded-3xl border border-border bg-card shadow-float scrollbar-thin ${className}`}
      ...
      >
        <div className="sticky top-0 z-10 mb-5 flex items-center justify-between rounded-t-3xl border-b border-border bg-card/95 px-5 py-4 backdrop-blur sm:px-7 sm:py-5">
          <div className="flex min-w-0 items-center gap-3">
            ...
            <div className="min-w-0"><h3 className="heading-font text-xl font-bold">{title}</h3>{description && <p className="mt-1 text-xs text-muted-foreground">{description}</p>}</div>
          </div>
          <button
            type="button"
            onClick={onClose}
